...
function GrowthDashboard({ data }: { data: NonNullable<ReturnType<typeof useGrowthAnalytics>["data"]> }) {
  ...
  return <div className="mt-7 space-y-9">
    <section className="grid gap-6 rounded-xl border border-border/55 bg-muted/[0.08] p-5 md:grid-cols-[minmax(0,1.2fr)_minmax(280px,.8fr)] md:p-6">
      <div><p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground">TRAINING LEVEL</p><div className="mt-2 flex flex-wrap items-baseline gap-x-3 gap-y-1"><span className="text-3xl font-semibold tracking-tight">Level {lifetime.level}</span><span className="text-sm text-primary">{lifetime.levelTitle}</span></div><p className="mt-1 text-[11px] text-muted-foreground">전체 훈련 누적 기준</p><p className="mt-3 text-sm text-muted-foreground">과정 중심 훈련 XP <span className="font-semibold tabular-nums text-foreground">{lifetime.totalXp.toLocaleString()}</span></p><div className="mt-4 h-2 overflow-hidden rounded-full bg-muted/35"><div className="h-full rounded-full bg-primary transition-[width]" style={{ width: `${Math.max(0, Math.min(100, lifetime.progressPercent))}%` }} /></div><div className="mt-2 flex justify-between text-[11px] text-muted-foreground"><span>{lifetime.currentLevelXp} XP</span><span>다음 레벨까지 {lifetime.nextLevelXp} XP</span></div></div>
      <dl className="grid grid-cols-2 gap-x-5 gap-y-4 border-t border-border/40 pt-5 md:border-l md:border-t-0 md:pl-6 md:pt-0"><Summary label="기간 내 완료 훈련" value={`${period.completedSessions}회`} /><Summary label="기간 내 거래" value={`${period.totalTrades}회`} /><Summary label="평균 Process Review" value={period.averageSessionAiScore === null ? "—" : period.averageSessionAiScore.toFixed(1)} /><Summary label="분석 기간" value={PERIOD_LABELS[period.key]} /></dl>
    </section>
    ...
    <section><SectionTitle title="훈련 습관" description="성과와 분리된 행동 지표입니다." /><div className="mt-4 space-y-4 border-y border-border/40 py-5"><Habit label="계획 후 행동" metric={period.planSessionRate} /><Habit label="근거 기록" metric={period.actionReasonRate} /><Habit label="리스크 사전 설정" metric={period.riskRuleSessionRate} /></div></section>
  </div>;
}
