...
import { accountFeatureAvailability, readStoredProfile } from "./myPageState";
    ...
    <div className="mx-auto w-full max-w-5xl space-y-5 px-4 py-6 sm:px-6 lg:py-8">
      ...
      <div className="grid gap-5 lg:grid-cols-[1.35fr_1fr]">
        <section aria-labelledby="training-heading" className="rounded-xl border border-border/55 bg-muted/[0.08] p-4 sm:p-5">
          ...
          {!accountFeatureAvailability.trainingHistory && (
            <div className="mt-4 rounded-lg border border-dashed border-border/50 px-4 py-6 text-center">
              <p className="text-sm font-medium">훈련 기록 조회는 준비 중입니다.</p>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">현재 서버에서 사용자별 세션 목록을 제공하지 않아 임의 통계를 표시하지 않습니다.</p>
              <Button asChild size="sm" className="mt-4"><Link to="/training">새 훈련 시작</Link></Button>
            </div>
          )}
        </section>
