...
export const trainingSessionApi = {
    ...
    createSession: (body: TrainingSessionCreateRequest) =>
        http.post<TrainingSessionCreateResponse>("/api/training/sessions", body).then(r => r.data),
    //세션 캔들 조회
    getSessionCandles: (sessionId: number) =>
        http.get<Candle[]>(`/api/training/sessions/${sessionId}/candles`).then(r => r.data),
    //다음 봉(1봉 진행)
    next: (sessionId: number) =>
        http.post<SessionProgressResponse>(`/api/training/sessions/${sessionId}/next`).then(r => r.data),
    //N봉 진행
    advance: (sessionId: number, steps: number) =>
        http.post<SessionProgressResponse>(`/api/training/sessions/${sessionId}/advance`, { steps }).then(r => r.data),
    //매수
    buy: (sessionId: number, qty: number) =>
        http.post<TradeResponse>(`/api/training/sessions/${sessionId}/trades/buy`, { qty } satisfies TradeRequest).then(r => r.data),
    //매도
    sell: (sessionId: number, qty: number) =>
        http.post<TradeResponse>(`/api/training/sessions/${sessionId}/trades/sell`, { qty } satisfies TradeRequest).then(r => r.data),
    //전량 매도
    sellAll: (sessionId: number) =>
        http.post<TradeResponse>(`/api/training/sessions/${sessionId}/trades/sell-all`).then(r => r.data),
    //리스크룰 조회
    getRiskRule: (sessionId: number) =>
        http.get<RiskRuleResponse>(`/api/training/sessions/${sessionId}/risk-rule`).then(r => r.data),
    //리스크룰 업서트(없으면 생성, 있으면 수정)
    upsertRiskRule: (sessionId: number, body: RiskRuleUpsertRequest) =>
        http.put<RiskRuleResponse>(`/api/training/sessions/${sessionId}/risk-rule`, body).then(r => r.data),
};