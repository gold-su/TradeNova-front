...
export function parseChartAiPayload(value: unknown): ChartAiPayload | null {
  ...
  if (payload.analysisScope !== "CHART" || typeof payload.score !== "number" || typeof payload.summary !== "string") return null;
  if (!isStringArray(payload.strengths) || !isStringArray(payload.warnings)) return null;
...
export function parseSessionAiPayload(value: unknown): SessionAiPayload | null {
  if (!value || typeof value !== "object") return null;
  const payload = value as Record<string, unknown>;
  if (payload.analysisScope !== "SESSION" || typeof payload.sessionId !== "number" || typeof payload.score !== "number" || typeof payload.summary !== "string") return null;
  if (!isStringArray(payload.strengths) || !isStringArray(payload.warnings)) return null;
